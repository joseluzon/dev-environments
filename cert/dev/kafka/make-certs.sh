...
KEYSTORE_CRED_FILE="$CURRENT_DIR/keystore_creds"
SSLKEY_CRED_FILE="$CURRENT_DIR/sslkey_creds"
TRUSTSTORE_CRED_FILE="$CURRENT_DIR/truststore_creds"
EXTFILE="-extfile ssl.cnf"

rm -f $KEYSTORE_SIGN_REQUEST_SRL
rm -f $KEYSTORE_SIGN_REQUEST
rm -f $KEYSTORE_SIGNED_CERT
rm -rf $CURRENT_DIR/keystore
rm -rf $CURRENT_DIR/truststore
...
openssl x509 -req -CA $CA_CERT_FILE -CAkey $trust_store_private_key_file \
  ...
  -days $VALIDITY_IN_DAYS -CAcreateserial $EXTFILE
...
echo
echo $PASS > $KEYSTORE_CRED_FILE
echo $PASS > $SSLKEY_CRED_FILE
echo $PASS > $TRUSTSTORE_CRED_FILE
